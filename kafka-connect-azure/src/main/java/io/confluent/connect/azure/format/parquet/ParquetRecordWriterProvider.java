...
import io.confluent.connect.azure.storage.S3ParquetOutputStream;
...
public class ParquetRecordWriterProvider extends RecordViewSetter
  ...
  ParquetRecordWriterProvider(AzBlobStorage storage, AvroData avroData) {
  ...
  private static class S3ParquetOutputFile implements OutputFile {
    ...
    S3ParquetOutputFile(AzBlobStorage storage, String filename) {
      this.storage = storage;
    ...
    public PositionOutputStream create(long blockSizeHint) {
      s3out = null; //(S3ParquetOutputStream) storage.create(filename, true, ParquetFormat.class);
      return s3out;
    }
