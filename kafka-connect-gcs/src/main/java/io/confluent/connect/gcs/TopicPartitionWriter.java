...
public class TopicPartitionWriter {
  ...
  private final RecordWriterProvider<GcsSinkConnectorConfig> writerProvider;
  ...
  private final GcsSinkConnectorConfig connectorConfig;
  private static final Time SYSTEM_TIME = new SystemTime();

  public TopicPartitionWriter(TopicPartition tp,
                              GcsStorage storage,
                              RecordWriterProvider<GcsSinkConnectorConfig> writerProvider,
                              Partitioner<FieldSchema> partitioner,
                              GcsSinkConnectorConfig connectorConfig,
  ...
  TopicPartitionWriter(TopicPartition tp,
    ...
    this.timestampExtractor = partitioner instanceof TimeBasedPartitioner
                                  ...
                                  : null;
    flushSize = connectorConfig.getInt(GcsSinkConnectorConfig.FLUSH_SIZE_CONFIG);
    topicsDir = connectorConfig.getString(StorageCommonConfig.TOPICS_DIR_CONFIG);
    rotateIntervalMs = connectorConfig.getLong(GcsSinkConnectorConfig.ROTATE_INTERVAL_MS_CONFIG);
    rotateScheduleIntervalMs =
        connectorConfig.getLong(GcsSinkConnectorConfig.ROTATE_SCHEDULE_INTERVAL_MS_CONFIG);
    if (rotateScheduleIntervalMs > 0) {
      timeZone = DateTimeZone.forID(connectorConfig.getString(PartitionerConfig.TIMEZONE_CONFIG));
    }
    timeoutMs = connectorConfig.getLong(GcsSinkConnectorConfig.RETRY_BACKOFF_CONFIG);
    ...
    zeroPadOffsetFormat = "%0"
        + connectorConfig.getInt(GcsSinkConnectorConfig.FILENAME_OFFSET_ZERO_PAD_WIDTH_CONFIG)
        + "d";
  ...
  private void commitFiles() {
    ...
    log.info("Files committed to GCS. Target commit offset for {} is {}", tp, offsetToCommit);
  }
