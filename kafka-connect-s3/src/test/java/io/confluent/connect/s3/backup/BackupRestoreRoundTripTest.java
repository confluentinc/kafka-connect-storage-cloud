...
public class BackupRestoreRoundTripTest {
  ...
  public void pristineSchemaNoConnectMetadata() throws Exception {
    ...
    assertNotNull("rawSchema present", rawSchema);
    assertEquals("rawSchema matches original",
        USER_V1_SCHEMA, rawSchema);
    assertFalse("no connect.name",
        rawSchema.contains("connect.name"));
    assertFalse("no connect.version",
        rawSchema.contains("connect.version"));
    assertFalse("no connect.parameters",
        rawSchema.contains("connect.parameters"));
  }

  // ========================== TEST 9 ==========================
  // Pristine schema with enum — survives despite Connect enum→STRING conversion

  static final String ENUM_SCHEMA =
      "{\"type\":\"record\",\"name\":\"WithEnum\","
      + "\"namespace\":\"com.example\","
      + "\"fields\":["
      + "{\"name\":\"id\",\"type\":\"int\"},"
      + "{\"name\":\"color\",\"type\":"
  ...
  public void pristineSchemaEnumPreserved() throws Exception {
    ...
    GenericRecord rec = new GenericData.Record(avro);
    rec.put("id", 1);
    rec.put("color", new GenericData.EnumSymbol(
        avro.getField("color").schema(), "GREEN"));
    ...
    assertTrue("rawSchema has enum type",
        rawSchema.contains("\"type\":\"enum\""));
    assertTrue("rawSchema has symbols",
        rawSchema.contains("\"symbols\""));
    assertFalse("rawSchema has no connect.parameters",
        rawSchema.contains("connect.parameters"));
  }
  ...
  static final String PROTO_SCHEMA =
      "syntax = \"proto3\";\n"
      + "package com.example;\n"
      + "message Order {\n"
      + "  int32 order_id = 1;\n"
  ...
  public void protobufRoundTrip() throws Exception {
    ...
    int schemaId = sourceSR.register(
        TOPIC + "-value", protoSchema);

    ProtobufConverter backupConv = new ProtobufConverter(sourceSR);
    Map<String, Object> cfg = new HashMap<>();
    cfg.put("schema.registry.url", "mock://");
    cfg.put("schema.backup.enabled", "true");
    backupConv.configure(cfg, false);

    com.google.protobuf.DynamicMessage msg =
        com.google.protobuf.DynamicMessage.newBuilder(
            protoSchema.toDescriptor())
        .setField(protoSchema.toDescriptor()
            .findFieldByName("order_id"), 42)
        .setField(protoSchema.toDescriptor()
            .findFieldByName("customer"), "Alice")
        .build();

    byte[] serialized = serializeProto(schemaId, msg);
    RecordHeaders headers = new RecordHeaders();
    SchemaAndValue backed = backupConv.toConnectData(
        TOPIC, headers, serialized);

    assertTrue(BackupWrapper.isWrapper(backed.schema()));
    Struct wrapper = (Struct) backed.value();
    assertEquals("PROTOBUF",
        wrapper.getString(BackupWrapper.FIELD_SCHEMA_TYPE));

    SchemaRegistryClient targetSR = new MockSchemaRegistryClient();
    ProtobufConverter restoreConv = new ProtobufConverter(targetSR);
    restoreConv.configure(cfg, false);
    byte[] restored = restoreConv.fromConnectData(
        TOPIC, headers, backed.schema(), backed.value());
    assertNotNull(restored);
    ...
    assertEquals("Alice", s.getString("customer"));
  }

  // ========================== TEST 11 ==========================
  // JSON Schema round-trip

  static final String JSON_SCHEMA_DEF =
  ...
  public void jsonSchemaRoundTrip() throws Exception {
    ...
    int schemaId = sourceSR.register(
        TOPIC + "-value", jsonSchema);

    JsonSchemaConverter backupConv = new JsonSchemaConverter(sourceSR);
    Map<String, Object> cfg = new HashMap<>();
    cfg.put("schema.registry.url", "mock://");
    cfg.put("schema.backup.enabled", "true");
    backupConv.configure(cfg, false);
    ...
    byte[] serialized = serializeJsonSchema(schemaId, jsonValue);
    RecordHeaders headers = new RecordHeaders();
    SchemaAndValue backed = backupConv.toConnectData(
        TOPIC, headers, serialized);
    ...
    SchemaRegistryClient targetSR = new MockSchemaRegistryClient();
    JsonSchemaConverter restoreConv = new JsonSchemaConverter(targetSR);
    restoreConv.configure(cfg, false);
    byte[] restored = restoreConv.fromConnectData(
        TOPIC, headers, backed.schema(), backed.value());
    assertNotNull(restored);
  ...
  private byte[] serializeProto(int schemaId,
      com.google.protobuf.DynamicMessage msg) throws Exception {
    ByteArrayOutputStream baos = new ByteArrayOutputStream();
    baos.write(0x00);
    baos.write(ByteBuffer.allocate(4).putInt(schemaId).array());
    baos.write(0x00); // message index (single message = 0)
    msg.writeTo(baos);
    return baos.toByteArray();
  }

  private byte[] serializeJsonSchema(int schemaId,
      com.fasterxml.jackson.databind.JsonNode value) throws Exception {
    ByteArrayOutputStream baos = new ByteArrayOutputStream();
    baos.write(0x00);
    baos.write(ByteBuffer.allocate(4).putInt(schemaId).array());
    baos.write(new com.fasterxml.jackson.databind.ObjectMapper()
